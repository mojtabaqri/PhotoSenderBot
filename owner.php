...
$data=[
    ...
    'companyphone'=>$input[25]//شماره هماهنگی شرکت 
     ...
     $message = sprintf("🌺مشتری گرامی،  بارنامه با محموله %s با تناژ %s به مقصد %s  در تاریخ %s توسط شرکت حمل و نقل آرام بار ثبت و صادر شد .✅\n👤نام راننده : %s\n📞شماره راننده :%s\n🔍پلاک خودرو : %s\n🚚نوع خودرو : %s\nلغو11", $data['bar'], $data['dtonaj'], $data['dest'], $time, $data['dname'], $data['dphone'], $data['dpluck'], $data['carType']);

try{  
    $message = "\n".$message;
    $lineNumber = 3000859975; 
    $receptor = $data['sahebbarphone'];
